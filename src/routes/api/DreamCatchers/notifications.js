...
router.post('/', async (req, res) => {
    try {
        const data = req.body;
        const notificationsRef = db.collection('notifications').doc(`${data.product_id}`);
        ...
        if(notificationsRef.exists) {
            ...
            res.status(200).json({ message: 'Updated Customer Notification' });
        }
        else{
            await notificationsRef.set({
                product_id: data.product_id,
            ...
            res.status(200).json({ message: 'Created Customer Notification' });
        }
    } catch (error) {
